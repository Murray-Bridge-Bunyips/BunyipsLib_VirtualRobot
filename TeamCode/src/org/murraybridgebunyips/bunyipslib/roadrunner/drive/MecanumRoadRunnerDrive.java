        ...
        for (DcMotorEx motor : motors) {
            if (motor instanceof Motor && runMode == DcMotor.RunMode.RUN_USING_ENCODER) {
                if (!((Motor) motor).getRunUsingEncoderController().isPresent())
                    throw new EmergencyStop("A configured motor is a Motor instance using RoadRunner and velocity encoders. You need to set your own controller to use via setRunUsingEncoderController() in your config.");
            } else {
