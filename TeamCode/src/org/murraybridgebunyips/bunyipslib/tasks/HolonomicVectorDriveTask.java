...
public class HolonomicVectorDriveTask extends ForeverTask {
    ...
    protected void periodic() {
        ...
        drive.setWeightedDrivePower(
                new Pose2d(
                        xLock != 0 && yLock != 0 ? -xController.calculate(twistedXError) : userX,
                        yLock != 0 && xLock != 0 ? -yController.calculate(twistedYError) : userY,
                        rLock != 0 ? -rController.calculate(angle) : userR
                )
        );

        //TODO: debug
        Dbg.log("HolonomicVectorDriveTask -> x: %, y: %, r: %", xLock, yLock, rLock);
    }
