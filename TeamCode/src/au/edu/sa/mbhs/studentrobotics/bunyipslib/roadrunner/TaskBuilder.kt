...
class TaskBuilder(private val constants: Constants, startPose: Pose2d, poseMap: PoseMap) {
    ...
    fun afterDisp(ds: Double, unit: Distance = Inches, a: Action) =
        apply { builder = builder.afterDisp(unit.of(ds).inUnit(Inches), a) }
    ...
    fun afterTime(dt: Double, unit: Time = Seconds, a: Action) =
        apply { builder = builder.afterTime(unit.of(dt).inUnit(Seconds), a) }
    ...
    fun turn(angle: Double, unit: Angle = Radians) =
        apply { builder = builder.turn(unit.of(angle).inUnit(Radians), turnConstraints) }
    ...
    fun turnTo(heading: Double, unit: Angle = Radians) =
        apply { builder = builder.turnTo(unit.of(heading).inUnit(Radians), turnConstraints) }
    ...
    fun lineToX(posX: Double, unit: Distance = Inches) =
        apply { builder = builder.lineToX(unit.of(posX).inUnit(Inches), velConstraints, accelConstraints) }
    ...
    fun lineToY(posY: Double, unit: Distance = Inches) =
        apply { builder = builder.lineToY(unit.of(posY).inUnit(Inches), velConstraints, accelConstraints) }
